    ...
    : encoder(nullptr), as_numpy(as_numpy)
{
    try
    {
        ...
        if (dtype == "uint8")
        {
           ...
           // convert =
             //   std::make_unique<celux::conversion::gpu::cuda::RGBToNV12<uint8_t>>();
        }
        else if (dtype == "float32")
        {
          ...
          //  convert =
            //    std::make_unique<celux::conversion::gpu::cuda::RGBToNV12<float>>();
        }
        else if (dtype == "float16")
        {
           ...
           // convert =
           //     std::make_unique<celux::conversion::gpu::cuda::RGBToNV12<__half>>();
        }
        else
        {
        ...
        encoder = std::make_unique<celux::Encoder>(filePath, props, useHardware, hwType,
                                                   std::move(convert));
