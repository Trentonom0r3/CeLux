...
Frame::Frame() : frame(av_frame_alloc())
{
    if (!frame)
    {
        throw FFException("Failed to allocate AVFrame");
    }
}
...
Frame::Frame(AVFrame* frame) : frame(frame)
{
    if (!frame)
    {
        throw FFException("Null AVFrame provided");
    }
}
...
Frame::Frame(const Frame& other) : frame(av_frame_clone(other.frame))
{
    if (!frame)
    {
        throw FFException("Failed to clone AVFrame");
    }
}
...
Frame& Frame::operator=(const Frame& other)
{
    if (this != &other)
    {
        ...
        if (!frame)
        {
            throw FFException("Failed to clone AVFrame during copy assignment");
        }
    }
    return *this;
}
...
uint8_t* Frame::getData(int plane) const
{
    ...
    {
        throw FFException("Invalid plane index: " + std::to_string(plane));
    }
...
void Frame::allocateBuffer(int align)
{
    ...
    {
        throw FFException("Failed to allocate buffer for AVFrame with alignment " +
                          std::to_string(align));
    }
}
...
void Frame::copyFrom(const Frame& other)
{
    ...
    {
        throw FFException("Failed to copy data from source AVFrame");
    }
    if (av_frame_copy_props(frame, other.frame) < 0)
    {
        throw FFException("Failed to copy properties from source AVFrame");
    }
}
...
void Frame::fillData(uint8_t* data, int size, int plane)
{
    ...
    if (size > maxSize)
    {
        throw FFException("Data size exceeds buffer capacity for plane " +
                          std::to_string(plane));
    }
