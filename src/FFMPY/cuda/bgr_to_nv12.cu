...
extern "C"
{
    ...
    __global__ void bgr_to_nv12_kernel_uchar(const unsigned char* __restrict__ bgrInput,
                                             int width, int height, int bgrStride,
                                             ...
                                             int yStride, int uvStride)
    {
        ...
        // Read RGB values
        int rgbIndex = y * bgrStride + x * 3;
        unsigned char R = bgrInput[rgbIndex + 0];
        unsigned char G = bgrInput[rgbIndex + 1];
        unsigned char B = bgrInput[rgbIndex + 2];
    ...
    __global__ void bgr_to_nv12_kernel_float(const float* __restrict__ bgrInput,
                                             int width, int height, int bgrStride,
                                             unsigned char* __restrict__ yPlane,
                                             unsigned char* __restrict__ uvPlane,
                                             int yStride, int uvStride)
    {
        ...
        int rgbIndex = y * bgrStride + x * 3;
        float R = bgrInput[rgbIndex + 0] * 255.0f;
        float G = bgrInput[rgbIndex + 1] * 255.0f;
        float B = bgrInput[rgbIndex + 2] * 255.0f;
    ...
    __global__ void bgr_to_nv12_kernel_half(const __half* __restrict__ bgrInput,
                                            int width, int height, int bgrStride,
                                            unsigned char* __restrict__ yPlane,
                                            unsigned char* __restrict__ uvPlane,
                                            int yStride, int uvStride)
    {
        ...
        // Read and denormalize RGB values
        int rgbIndex = y * bgrStride + x * 3;
        float R = __half2float(bgrInput[rgbIndex + 0]) * 255.0f;
        float G = __half2float(bgrInput[rgbIndex + 1]) * 255.0f;
        float B = __half2float(bgrInput[rgbIndex + 2]) * 255.0f;
                     ...
                     int yStride, int uvStride, cudaStream_t stream = 0)
    {
        ...
        bgr_to_nv12_kernel_uchar<<<grid, block, 0, stream>>>(
            bgrInput, width, height, bgrStride, yPlane, uvPlane, yStride, uvStride);
                           ...
                           int uvStride, cudaStream_t stream = 0)
    {
        ...
        bgr_to_nv12_kernel_float<<<grid, block, 0, stream>>>(
            bgrInput, width, height, bgrStride, yPlane, uvPlane, yStride, uvStride);
                          ...
                          int uvStride, cudaStream_t stream = 0)
    {
        ...
        bgr_to_nv12_kernel_half<<<grid, block, 0, stream>>>(
            bgrInput, width, height, bgrStride, yPlane, uvPlane, yStride, uvStride);
