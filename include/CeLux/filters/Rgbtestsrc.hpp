...
class Rgbtestsrc : public FilterBase {
public:
    /**
     ...
     * Default: false
     */
    void setComplement(bool value);
    bool getComplement() const;

    Rgbtestsrc(std::pair<int, int> size = std::make_pair<int, int>(0, 1), std::pair<int, int> rate = std::make_pair<int, int>(0, 1), int64_t duration = 0, std::pair<int, int> sar = std::make_pair<int, int>(0, 1), bool complement = false);
    virtual ~Rgbtestsrc();
